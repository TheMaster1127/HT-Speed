...
// Recursive include loader (expands include "file.hts" directly into memory)
...
static void run_compiler(void) {
    ...
    // Reserve first 16 bytes of data pool for __orig_argc and __orig_argv
    C.data_len = 16;
    ...
    while (cur_tok.kind != TOK_EOF) {
        // Struct definition: struct Point { ... } OR struct Point ... subout
        if (cur_tok.kind == TOK_STRUCT) {
            ...
            int uses_brace = (cur_tok.kind == TOK_LBRACE);
            if (uses_brace) next_token();

            while (cur_tok.kind != TOK_RBRACE && cur_tok.kind != TOK_SUBOUT && cur_tok.kind != TOK_EOF) {
            ...
            if (uses_brace) expect(TOK_RBRACE);
            else expect(TOK_SUBOUT);
            continue;
        }

        // Top-level Global Variables (before main)
        if (cur_tok.kind == TOK_TYPE_INT || cur_tok.kind == TOK_TYPE_STR || cur_tok.kind == TOK_TYPE_BOOL) {
        ...
        if (cur_tok.kind == TOK_MAIN) {
	        ...
	        // Clean stack setup: push rbp; mov rbp, rsp; sub rsp, 4096
	        emit_u8(0x55);
	        emit_u8(0x48); emit_u8(0x89); emit_u8(0xE5);
	        emit_u8(0x48); emit_u8(0x81); emit_u8(0xEC);
	        emit_u32(4096);
